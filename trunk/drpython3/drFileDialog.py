...
import os, os.path, re, stat, time, sys, locale
import wx
	...
	def SetDirectoryTo(self, directory, filter="*", updatedirectories=True):
		...
		try:
			if wx.USE_UNICODE:	
				...
				thelist = []
				if (len(self.grandparent.prefs.defaultencoding) > 0) and self.grandparent.PLATFORM_IS_WIN:
					for u in uthelist:
						try:
							thelist.append(unicode(u, self.grandparent.prefs.defaultencoding))
						except:
							print 'Error Decoding Directory Entry with default encoding:', self.grandparent.prefs.defaultencoding
				else:
					for u in uthelist:
						if not drEncoding.CheckAscii(u):
							print 'Error Decoding Directory Entry:  Not Ascii.'
						else:				
							thelist.append(u)
			else:
				...
				if additem:
					if refilter.match(item):
						...
						if self.IsDetailed:
							...
							self.DataArray.append((item, size, type, mtime, mmode))	
							
							self.SetStringItem(i, 1, str(size))
							self.SetStringItem(i, 2, type)
							self.SetStringItem(i, 3, mtime)
							self.SetStringItem(i, 4, mmode)
		except:
...
class drFileDialog(wx.Dialog):
	def __init__(self, parent, title, wildcard, point=wx.DefaultPosition, size=wx.DefaultSize, IsASaveDialog=0, MultipleSelection=0, ShowRecentFiles=0):
		...
		if len(self.parent.prefs.defaultencoding) > 0:
	...
	def OnAddBookmark(self, event):
		try:
			bookfile = self.parent.userpreferencesdirectory + "/bookmarks.dat"
	...
	def OnbtnDelete(self, event):
		...
		if os.path.exists(filename) and not os.path.isdir(filename):
			msg = 'Delete "' + filename + '"?'
			d = wx.MessageDialog(self, msg, "Delete File?", wx.YES_NO | wx.ICON_QUESTION)
			answer = d.ShowModal()
			d.Destroy()
			if answer == wx.ID_YES:
				try:
