...
class InterfaceModePanel(PrefsPanel):
	
	def __init__(self, parent, id, name):
		PrefsPanel.__init__(self,parent,id,name,2)
		self.label = wx.StaticText(self, -1, "DrPython must be restarted to reflect changes", (20, 45))
		self.modeBoxes = wx.RadioBox(self, -1, "Interface Mode:", (20, 70), wx.DefaultSize, ["Advanced Mode", "Beginner Mode"], 1, wx.RA_SPECIFY_COLS)
		self.chkDisplayAtStart = wx.CheckBox(self, -1, "Don't display at start", (20, 150))
	...
	def reset(self):
		if self.grandparent.prefs.mode == "advanced":
			self.modeBoxes.SetSelection(0)
		else:
			self.modeBoxes.SetSelection(1)
		
		self.chkDisplayAtStart.SetValue(self.grandparent.prefs.save)
...
class drPrefsDialog(wx.Dialog):
	...
	def __init__(self, parent, id, title):
		...
		self.pnlInterfaceMode = InterfaceModePanel(self.lbPrefs, -1, "Interface Mode")
		...
		self.lbPrefs.AddPage(self.pnlInterfaceMode, "Interface Mode")
	...
	def GetPreferences(self):
		...
		#Interface Mode
		if self.pnlInterfaceMode.chkDisplayAtStart.GetValue():
			self.prefs.save = 1
		else:
			self.prefs.save = 0
		
		if self.pnlInterfaceMode.modeBoxes.GetSelection():
			self.prefs.mode = "beginner"
		else:
			self.prefs.mode = "advanced"
